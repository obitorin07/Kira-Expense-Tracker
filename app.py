...
def reset_budget():
    ...
    if today_day == 1:  # Reset budget only on the 1st
        cursor.execute(
            "INSERT INTO budget (month_year, total_budget) VALUES (%s, %s) "
            "ON DUPLICATE KEY UPDATE total_budget = 12000",
            (current_month, 12000)
        )
        conn.commit()
    conn.close()
...
def get_budget():
    ...
    conn.close()
    return Decimal(result[0]) if result else Decimal(12000)

# Get total spent from 1st to the current date (including all of today)
def get_total_spent():
    conn = get_db_connection()
    cursor = conn.cursor()

    today = datetime.date.today()
    # 1st of month at 00:00:00
    start_datetime = datetime.datetime.combine(today.replace(day=1), datetime.time.min)
    # Today at 23:59:59.999999
    end_datetime   = datetime.datetime.combine(today, datetime.time.max)

    cursor.execute(
        "SELECT SUM(amount) FROM expenses WHERE date_time BETWEEN %s AND %s",
...
def add_expense(date_time, category, amount, description, payment_method):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO expenses (date_time, category, amount, description, payment_method) "
        "VALUES (%s, %s, %s, %s, %s)",
...
st.markdown(
    "<h2 style='text-align: center; color: #4CAF50;'>Expense Tracker</h2>",
...
st.markdown(
    f"<h4 style='text-align: center;'>📅 {current_day}, {current_date} &nbsp;&nbsp;&nbsp; "
    f"Budget: ₹{monthly_budget}</h4>",
    unsafe_allow_html=True
)
...
if remaining_budget >= 0:
    st.markdown(
        f"<h4 style='text-align: center;'>Total Spent: ₹{total_spent} &nbsp;&nbsp;&nbsp; "
        f"Remaining: ₹{remaining_budget}</h4>",
        unsafe_allow_html=True
    )
else:
    st.markdown(
        f"<h4 style='text-align: center;'>Total Spent: ₹{total_spent} &nbsp;&nbsp;&nbsp; "
        f"<span style='color: red;'>Remaining: ₹{remaining_budget}</span></h4>",
        unsafe_allow_html=True
    )

# Add new expense
st.subheader("➕ Add an Expense")
date_input = st.date_input("Date:", current_date)
...
category       = st.selectbox(
    "Category:",
    ["Rent & PG", "Food & Groceries", "Transport", "Utilities & Bills",
     "Shopping", "Entertainment", "Health & Medical", "Education & Learning",
     "Miscellaneous", "Savings & Investments"]
)
...
if st.button("Add Expense"):
    full_datetime = datetime.datetime.combine(date_input, selected_time)
